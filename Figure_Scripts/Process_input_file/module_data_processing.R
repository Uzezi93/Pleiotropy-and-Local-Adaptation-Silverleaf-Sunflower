...
# =============================================================================
bed_dir   <- "/project/pi_brook_moyers_umb_edu/Uzezi_argo/raw_fastq/variants/chunks/analysis_vcf_files/enriched_modules_gene_lists/"
...
readr::write_tsv(north_subset, file.path(outdir, "north_pi_watterson_by_color.tsv"))
readr::write_tsv(coast_subset, file.path(outdir, "coast_pi_watterson_by_color.tsv"))
...
readr::write_tsv(fst_by_module, file.path(outdir, "fst_by_color_from_bed.tsv"))
readr::write_tsv(dxy_by_module, file.path(outdir, "dxy_by_color_from_bed.tsv"))
...
BASE <- "/project/pi_brook_moyers_umb_edu/Uzezi_argo/raw_fastq/variants/chunks/analysis_vcf_files/enriched_modules_gene_lists/angsd_per_gene_out/"

# Modules and pops to scan (adjust if needed)
modules <- c("blue_genes","pink_genes","yellow_genes","red_genes","green_genes","control_pool_all_genes")
pops    <- c("coast","north")
...
outfile_all   <- file.path(out_dir, "angsd_theta_by_module_by_population.tsv")
outfile_coast <- file.path(out_dir, "angsd_theta_by_module_coast.tsv")
outfile_north <- file.path(out_dir, "angsd_theta_by_module_north.tsv")
