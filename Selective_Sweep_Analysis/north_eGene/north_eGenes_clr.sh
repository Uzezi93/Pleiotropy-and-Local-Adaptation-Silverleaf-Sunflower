...
bcftools query -f '%CHROM\n' north_eGenes2.vcf.gz | uniq > saffolds.txt

 cat saffolds.txt | while read CONTIG; do
    vcftools --gzvcf north_eGenes2.vcf.gz  --chr $CONTIG --recode --recode-INFO-all --out ./scaff/$CONTIG.out
done
...
#head -n 1 Ha412HOChr17.out.recode_clr.out  > north_eGenes2_clr.out

#tail -n +2 -q *clr.out >> north_eGenes2_clr.out

cat *_clr.out > temp_clr_combined.txt

awk '/location[[:space:]]+LR[[:space:]]+alpha/ {if (seen++) next} {print}' temp_clr_combined.txt > north_eGenes2_clr.out
