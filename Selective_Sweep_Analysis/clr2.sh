#!/usr/bin/env bash
#SBATCH -J sf2_all_modules_by_gene
#SBATCH -c 5
...
# -------------------- PATHS --------------------
ROOT="/project/pi_brook_moyers_umb_edu/Uzezi_argo/raw_fastq/variants/chunks/analysis_vcf_files/vcf_by_gene_sets"
VCF_DIR="${ROOT}/by_pop_vcfs"                         # expects <module>.<pop>.vcf.gz
BED_DIR="${ROOT}"                                     # expects per-module *_genes.coords.bed
OUTROOT="${PWD}/sf2_all_modules_by_gene_out"          # results root
MANIFEST="${OUTROOT}/task_manifest.tsv"               # built once (tab-delimited, with header)
SF2="/project/pi_brook_moyers_umb_edu/SF2/SweepFinder2"

mkdir -p "${OUTROOT}"

# -------------------- MODULE & BED MAPPING --------------------
# keys must match the VCF prefix "<module>.<pop>.vcf.gz"
# and the BED filenames listed on the right.
declare -A BED_FOR=(
  [control_genes]="${BED_DIR}/control_genes.coords.bed"
  [eGenes]="${BED_DIR}/eGene_genes.coords.bed"
  [eQTL_genes]="${BED_DIR}/eQTL_genes.coords.bed"
  [lfmm_genes]="${BED_DIR}/lfmm_genes.coords.bed"
  [pcadapt_genes]="${BED_DIR}/pcadapt_genes.coords.bed"
  [shared_genes]="${BED_DIR}/shared_genes.coords.bed"
)

# modules and populations to process
MODULES=(control_genes eGenes eQTL_genes lfmm_genes pcadapt_genes shared_genes)
POPS=(coast north)
...
if [[ ! -s "${MANIFEST}" ]]; then
  ...
  for mod in "${MODULES[@]}"; do
    ...
    for pop in "${POPS[@]}"; do
      ...
      # Expect BED with at least 4 columns: chrom start end gene (extra columns okay)
...
# (manifest header is line 1; data rows start at 2)
