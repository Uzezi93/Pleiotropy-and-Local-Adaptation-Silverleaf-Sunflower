...
#outlier_genes <- unique(append(lfmm_genes, pcadapt_genes))

all_genes <- read.delim("./GCF_002127325.2_HanXRQr2.0-SUNRISE_gene_ontology.gaf", skip = 8, header = TRUE)
...
write.table(x, file = "../Helianthus_argophyllus_project/argo_plots/PLOS_genetics_figures/LFMM_Gene_over-representation.txt", sep = "\t", quote = F)

p1 <- barplot(x, main="LFMM Genes Over Representation Analysis") +
  ggtitle("Gene Over Representation Analysis for LFMM Outliers")

# Save the ggplot as a .tiff file
ggsave("/project/pi_brook_moyers_umb_edu/Uzezi_argo/Helianthus_argophyllus_project/argo_plots/PLOS_genetics_figures/LFMM_gene_over_representation_barplot.tiff", plot = p1, device = "tiff", width = 7, height = 4)

p2 <- dotplot(x) +
  ggtitle("Gene Over Representation Analysis for LFMM Outliers")
ggsave("/project/pi_brook_moyers_umb_edu/Uzezi_argo/Helianthus_argophyllus_project/argo_plots/PLOS_genetics_figures/LFMM_gene_over_representation_dotplot.tiff", plot = p2, device = "tiff", width = 7, height = 4)

#-------------------------------------------------------------------------------------
# Enrichnment for PCAdapt outliers
y = enricher(pcadapt_genes, TERM2GENE=term2gene)
head(summary(y))

# Save gene over representation analysis table
write.table(y, file = "../Helianthus_argophyllus_project/argo_plots/PLOS_genetics_figures/PCAdapt_Gene_over-representation.txt", sep = "\t", quote = F)

p3 <- barplot(y, main="PCAdapt Genes Over Representation Analysis") +
  ggtitle("Gene Over Representation Analysis for PCAdapt Outliers")

# Save the ggplot as a .tiff file
ggsave("/project/pi_brook_moyers_umb_edu/Uzezi_argo/Helianthus_argophyllus_project/argo_plots/PLOS_genetics_figures/PCAdapt_gene_over_representation_barplot.tiff", plot = p3, device = "tiff", width = 7, height = 4)

p4 <- dotplot(y) +
  ggtitle("Gene Over Representation Analysis for PCAdapt Outliers")
ggsave("/project/pi_brook_moyers_umb_edu/Uzezi_argo/Helianthus_argophyllus_project/argo_plots/PLOS_genetics_figures/PCAdapt_gene_over_representation_dotplot.tiff", plot = p4, device = "tiff", width = 7, height = 4)
