...
bcftools query -f '%CHROM\n' north_control2.recode.vcf.gz | uniq > saffolds.txt

cat saffolds.txt | while read CONTIG; do
    vcftools --gzvcf north_control2.recode.vcf.gz --chr $CONTIG --recode --recode-INFO-all --out ./scaff/$CONTIG.out
done
...
awk '/location[[:space:]]+LR[[:space:]]+alpha/ {if (seen++) next} {print}' temp_clr_combined.txt > north_control2_clr.out
