...
bcftools query -f '%CHROM\n' coast_eGenes2.vcf.gz | uniq > saffolds.txt

 cat saffolds.txt | while read CONTIG; do
    vcftools --gzvcf coast_eGenes2.vcf.gz  --chr $CONTIG --recode --recode-INFO-all --out ./scaff/$CONTIG.out
done
...
# head -n 1 Ha412HOChr17.out.recode_clr.out  > coast_eGenes_clr.out

# tail -n +2 -q *clr.out >> coast_eGenes_clr.out

cat *_clr.out > temp_clr_combined.txt

awk '/location[[:space:]]+LR[[:space:]]+alpha/ {if (seen++) next} {print}' temp_clr_combined.txt > coast_eGenes2_clr.out
